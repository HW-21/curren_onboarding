...
describe('Company Description', () => {
  ...
  it('should not be able to have sanctiond country as payment inward/outward countries', () => {
    const inwardCountry = 'Afghanistan'
    const outwardCountry = 'Belarus'
    const companyDescriptionForm = new CompanyDescriptionForm()

    companyDescriptionForm.checkIsCurrentForm()
    companyDescriptionForm.checkValues(values)
    companyDescriptionForm.changeValues({
      expectedIncomingCountries: inwardCountry,
      expectedOutgoingCountries: outwardCountry,
    })
    companyDescriptionForm.continue()

    companyDescriptionForm.checkErrors({
      expectedIncomingCountries: `You have selected these high risk or sanctioned countries: ${inwardCountry}`,
      expectedOutgoingCountries: `You have selected these high risk or sanctioned countries: ${outwardCountry}`,
    })
